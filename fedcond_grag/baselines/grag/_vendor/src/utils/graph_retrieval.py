...
# NOTE (fedrag fork): upstream loads the sbert model (~1.3GB download) at
# *module import time* unconditionally, even though it's only actually used
# when augment != "none" (path/triplet/trunk augmentation, which we don't
# use by default). Load it lazily on first use instead -- same behavior,
# just deferred; `augment="none"` (this project's default call) never
# touches it at all.
...
def _get_sentence_model():
    ...
    if _sentence_model_cache is None:
        model, tokenizer, _dev = load_model[sentence_lm]()
        _sentence_model_cache = (model, tokenizer)
...
def get_triplets(subgraph):
    ...
    # NOTE (fedrag fork): upstream hardcoded `.view(-1, 1024)`, assuming
    # GRAG's paper-default 1024-dim (all-roberta-large-v1) embeddings. This
    # project uses 384-dim (all-MiniLM-L6-v2) throughout, matching the
    # baselines/gretriever precedent -- use the actual embedding dim instead
    # of a hardcoded one so this works for any embedder.
    combined_attr = torch.cat([subgraph.x, subgraph.edge_attr.view(-1, subgraph.x.size(-1))], dim=0)
...
def retrive_on_graphs(graph, q_emb, textual_nodes, textual_edges, topk=10, k = 2, topk_entity = 5, augment = "none", sims = None):
    ...
    if augment != "none":
        sentence_model, sentence_tokenizer = _get_sentence_model()
        subg_embs = load_text2embedding[sentence_lm](sentence_model, sentence_tokenizer, device, flatten_graphs)

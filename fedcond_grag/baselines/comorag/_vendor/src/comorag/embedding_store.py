...
class EmbeddingStore:
    ...
    def _load_data(self):
        if os.path.exists(self.filename):
            ...
            # NOTE (fedrag fork): upstream never set these two here (only in
            # the "loaded from disk" branch above), so any code path that
            # reads them on a still-empty store (e.g. ComoRAG.tri_retrieve's
            # `self.level_store.text_to_hash_id` when the timeline/level-0
            # store hasn't had anything inserted into it yet) hits an
            # AttributeError. Matches the loaded-branch's empty case.
